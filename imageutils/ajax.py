...
def save_image(request):
    ...
    user.avatar.save(filename, InMemoryUploadedFile(
        file=imagefile,
        field_name=None,
        name=filename,
        content_type=mime,
    ...
    return JsonResponse({'is_valid': True, 'url': user.avatar.url})
...
def upload_avatar(request):
    ...
    if form.is_valid():
        ...
        user.avatar.save(filename, photo.file, save=True)
        data = {'is_valid': True, 'name': user.avatar.name, 'url': user.avatar.url}
    else:
        data = {'is_valid': False}
        if form.errors:
